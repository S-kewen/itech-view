...
$(document).ready(function () {
    $.ajax({
        url: "//itech-api.iskwen.com/api/getYuntechFlowSetInfo",
        type: "post",
        dataType: "json",
        ...
        success: function (msg) {
            if (msg.status === 0) {
                $("#ip").val(msg.ip);
                $("#warnValue").val(msg.warnValue);
                $("#warnMail").val(msg.warnMail);
                $("#warnPhone").val(msg.warnPhone);
                if (msg.mailState === 1) {
                    $("input[name='mailState'][value='1']").attr("checked", true);
                } else {
                    $("input[name='mailState'][value='2']").attr("checked", true);
                }
                if (msg.phoneState === 1) {
                    $("input[name='phoneState'][value='1']").attr("checked", true);
                } else {
                    $("input[name='phoneState'][value='2']").attr("checked", true);
                }
                $("#mailInterval").val(msg.mailInterval);
                $("#phoneInterval").val(msg.phoneInterval);
...
function post() {
    ...
    $.ajax({
        url: "//itech-api.iskwen.com/api/updateYuntechFlowSet?id=" + getUrlValue("id"),
        type: "post",
        dataType: "json",
        headers: {'Authorization': getCookie("token")},
